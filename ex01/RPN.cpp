...
int performOperation(int first, int second, char operator_char)
{
    ...
    {
        if (second == 0)
            throw RPN::DivisionByZero("Cannot perform division by zero: " + std::to_string(first) + '/' + std::to_string(second));
        return first / second;
    }
...
int RPN::evaluate(const std::string& expression)
{
    ...
    while (true)
    {
        ...
        catch(const std::exception&)
        {
            ...
            if (remaining_pos < cur_expression.length() && isOperator(cur_expression[remaining_pos]))
            {
                ...
                if (m_nums.size() < 2)
                    throw InvalidExpression("Invalid expression (operation cannot be performed on fewer than 2 numbers): " + expression);
                ...
                throw InvalidExpression("Invalid expression (unknown operator / elements found): " + expression);
        }
        //  
    }
    ...
    if (!cur_expression.empty())
        throw InvalidExpression("Something went wrong (unevaluated string) while evaluating expression: " + expression);
